...
import {EditMyEvent, IEditMyEvent} from '../../models/content/content-edit-model';
...
export class EditMyEventComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    }), takeUntil(this.destroy$)).subscribe(content => {
      //Вписать значения в форму
      this.title.patchValue(content.title);
      this.body.patchValue(content.body);
      this.categoryId.patchValue(content.categoryId);
      this.myDateTime.patchValue(content.myDateTime);
      this.tagstr = "";
